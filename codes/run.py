...
def main(args):
    ...
    if args.trans_test != 'test.txt':
        trans_triples = read_triple(os.path.join(args.data_path, 'trans_test/trans_closure.txt'), entity2id, relation2id)
        logging.info('Transitive test:')
        logging.info('#trans: %d' % len(trans_triples))
        test_triples = read_triple(os.path.join(args.data_path, args.trans_test), entity2id, relation2id)
        logging.info('#test_trans: %d' % len(test_triples))
        all_true_triples += trans_triples
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.log_steps == 0:
                ...
                #                 log_metrics('Training average', step, metrics, fp)
                training_logs = []
